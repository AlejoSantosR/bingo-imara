...
function installCss(){if(document.getElementById('bingoLiveFlowCss'))return;const s=document.createElement('style');s.id='bingoLiveFlowCss';s.textContent=`
 ...
 @media(max-width:620px){.bingo-live-card{padding:28px 16px}.bingo-live-row{align-items:flex-start;flex-direction:column}.bingo-live-clock{width:72px;height:72px;font-size:30px}}
...
function sameCandidates(a,b){const x=(a||[]).map(c=>String(c.card_id)).sort().join('|'),y=(b||[]).map(c=>String(c.card_id)).sort().join('|');return x===y;}
...
function renderLive(){
 ...
 if(show.type==='bingo_countdown'){
   ...
   o.classList.remove('hidden');o.innerHTML=`<div class="bingo-live-card"><div class="bingo-live-kicker">BINGO IMARA · VALIDACIÓN DE RONDA</div><div class="bingo-live-main" style="font-size:clamp(46px,8vw,94px)">${labels[stage]}</div>${who?`<div class="bingo-live-name" style="font-size:clamp(20px,3.5vw,38px)">${who}</div>`:''}<div class="bingo-live-clock">${remaining}</div><div class="bingo-live-sub">${ready?'Conteo terminado · esperando decisión del presentador':'3 segundos para recibir otro BINGO'}</div>${ready&&!IS_PUBLIC&&isAdmin()?`<div class="bingo-live-actions">${cs.length===1?`<button class="btn good" data-live-confirm="${esc(cs[0].card_id)}">✅ Confirmar BINGO</button>`:`<button class="btn warn" id="bingoLiveTie">🔥 Desempatar ${cs.length} BINGOS</button>`}<button class="btn bad" id="bingoLiveContinue">▶ Continuar juego</button></div>`:''}</div>`;return;
 }
...
function patchOldCenter(){const f=document.querySelector('#bingoControl2026 .footer-note');if(f)f.textContent='Cada paso dura 3 segundos. Al terminar: confirmar, continuar el juego o desempatar si hay más de un BINGO válido.';}
...
async function publishValidClaims(){
 const vc=validClaims(),sig=vc.map(c=>String(c.card_id)).sort().join('|');
 ...
 try{
   ...
   if(current.type==='bingo_countdown'){
     const merged=mergeCandidates(current.candidates,vc);if(!sameCandidates(merged,current.candidates))await setShow({...current,candidates:merged});else{liveShow=current;renderLive();}
   }else{
...
async function start123(){
 ...
 const cs=unionCandidates();if(!cs.length){alert('No hay un BINGO válido para iniciar el conteo.');return;}
 await setShow({type:'bingo_countdown',started_at:new Date().toISOString(),interval_ms:COUNT_MS,round_name:(typeof state!=='undefined'&&state.round?.name)||'Ronda',candidates:cs.map(c=>({card_id:c.card_id,buyer_alias:c.buyer_alias||''}))});
 document.getElementById('refreshBingo2026')?.click();
}
...
function wire(){document.addEventListener('click',e=>{
 ...
 if(t.closest?.('#bingoLiveStart123')){e.preventDefault();start123();return;}
...
function startPublic(){refreshPublicShow();setInterval(refreshPublicShow,POLL_MS);if(token()){refreshClaims();setInterval(()=>{if(watchClaims)refreshClaims();},POLL_MS);}}
function waitAdmin(n=0){if(isAdmin()&&token()){startAdminWatcher();renderDashboard();return;}if(n<40)setTimeout(()=>waitAdmin(n+1),500);}
