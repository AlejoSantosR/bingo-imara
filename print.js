function printableCardHtml(c){
  ...
  return `<article class="ticket">
    <header class="ticketTop">
      ...
      <div class="numberBox"><span>Cartón</span><strong>${escapeHtml(c.id)}</strong></div>
    </header>
    <div class="priceLine"><span>Valor: <strong>${money(state.settings.price)}</strong></span><span>Modo 1–${state.settings.ballMax} · Premios sorpresa 🎁</span></div>
    <div class="pgrid">${cells}</div>
    <footer class="ticketFoot">
      <span>Comprador: <b>${escapeHtml(c.buyer||'________________________')}</b></span>
      <span>Solo participa si está PAGADO</span>
    </footer>
...
function openBatchPrint(ids,perPage=2,existingWindow=null){
  ...
  const w=existingWindow||window.open('','BingoIMARALote');
  if(!w){alert('El navegador bloqueó la ventana de impresión. Habilita ventanas emergentes para este archivo.');return;}
  ...
  w.document.open();
  w.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Cartones ${escapeHtml(state.settings.title)}</title><style>
  ...
  </style></head><body><div class="toolbar"><div><strong>${cards.length} cartones listos</strong> <small>· ${per} por hoja</small></div><button onclick="window.print()">🖨️ Imprimir / Guardar PDF</button></div><main class="pages">`);
  for(let i=0;i<cards.length;i+=per){
    w.document.write('<section class="sheet">');
    cards.slice(i,i+per).forEach(c=>w.document.write(printableCardHtml(c)));
