/* Bingo IMARA · puente de aviso inmediato de BINGO móvil · SAFE 2026
   ...
   Todo canto llega al Admin; solo un BINGO validado por servidor pasa a pantalla pública. */
(function(){
'use strict';
...
let lastPublicSig='',publishing=false;
...
function validClaims(list){return pendingClaims(list).filter(x=>x.valid===true);}
...
function renderDashboard(list){
 ...
 box.innerHTML=`<div style="font-size:16px;font-weight:1000;color:#ffe49a">📣 ${pending.length===1?'BINGO anunciado · revisar':`${pending.length} BINGOS anunciados · revisar`}</div><div class="muted" style="margin:4px 0 10px">El participante ya avisó BINGO. El Admin recibe todos los avisos; la pantalla pública solo se activa si el servidor confirma que la figura se cumple.</div>${pending.map(c=>`<div style="display:flex;justify-content:space-between;gap:10px;align-items:center;padding:10px 11px;margin-top:7px;border-radius:14px;border:1px solid var(--line);background:#10182a"><div><strong>📱 ${esc(c.buyer_alias||c.card_id)}</strong><br><small class="muted">${esc(c.card_id)} · ${c.valid===true?'✅ figura detectada · confirmar':'🕓 pendiente / no cumple todavía'}</small></div><button class="mini" data-imara-immediate-review="${esc(c.card_id)}">🔎 Revisar cartón</button></div>`).join('')}`;
}
async function publishPending(list){
 renderDashboard(list);
 const ready=validClaims(list),nextSig=sig(ready);
 if(!nextSig){lastPublicSig='';return;}
 if(nextSig===lastPublicSig||publishing||!isAdmin()||!token())return;
 lastPublicSig=nextSig;publishing=true;
 try{
   ...
   if(current.type==='bingo_live_claim')for(const c of current.candidates||[])if(c?.card_id&&c.valid===true)merged.set(String(c.card_id),c);
   for(const c of ready)merged.set(String(c.card_id),{card_id:c.card_id,buyer_alias:c.buyer_alias||'',valid:true});
